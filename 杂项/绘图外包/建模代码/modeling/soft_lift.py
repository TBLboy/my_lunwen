"""Configurable lift library for the soft-platform 2D xy state."""
...
@dataclass
class SoftLift:
    """A fixed-dimension nonlinear dictionary for normalized 2D xy states."""

    poly_degree: int = 4
    rbf_grid_size: int = 13
    rbf_sigma_scale: float = 0.8
    ...
    def fit(self, x: np.ndarray) -> "SoftLift":
        ...
        # Keep the numerical transform stable while fitting centers to the
        # observed working range rather than assuming a fixed sensor range.
    ...
    def report(self, sample: np.ndarray | None = None) -> dict:
        report = {
            ...
            "rbf_sigma": self.rbf_sigma.tolist(),
            "groups": {
                "constant": 1,
                "linear": 2,
                "polynomial": self.dimension - 3 - self.rbf_centers.shape[0] - 4 * len(self.trig_frequencies),
                "rbf": int(self.rbf_centers.shape[0]),
                "trig": 4 * len(self.trig_frequencies),
            },
        }
        if sample is not None:
            feature = self.transform(sample)
            singular_values = np.linalg.svd(feature, compute_uv=False)
            report["sample_condition_number"] = float(
                singular_values[0] / max(singular_values[-1], np.finfo(float).tiny)
            )
            report["sample_finite"] = bool(np.isfinite(feature).all())
        return report
...
def default_soft_lift(x_train: np.ndarray) -> SoftLift:
    """Fit the default approximately 200-dimensional lift."""

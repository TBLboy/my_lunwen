%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Elsevier manuscript for Robotics and Autonomous Systems (RAS)
% Prepared with the official elsarticle class.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[final,3p,twocolumn,times]{elsarticle}

% Use the preprint option when preparing a single-column submission version.
...
\title{Feature-Selection-Based Koopman Modeling and Robust LQR Control of Robots}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{pictures/figure2_modeling_accuracy.pdf}
...
As shown in Table~\ref{tab:modeling_accuracy}, FS-EDMD achieves the lowest full-state prediction errors on the robotic arm, with MSE, RMSE, and MAE of 0.3520, 0.5933, and 0.3202, respectively. These correspond to reductions of 40.7\%, 23.0\%, and 29.6\% over EDMDDL, and 51.5\%, 30.3\%, and 36.2\% over standard EDMD. On the soft platform, the FS-EDMD RMSE for test trajectory 66 is 0.7889, compared with 0.8965 for standard EDMD and 1.2927 for EDMDDL.
...
Trajectory tracking experiments were conducted in the joint space using sinusoidal and irregular references. The Koopman-LQR controller was implemented with the FS-EDMD model and the feedforward-feedback structure in Section~\ref{sec:lqr}. The weighting matrices were set to $\Q=\diag(100,10,100,10,100,10)$ and $\Rmat=\I_3$, and a PID controller with empirically tuned gains $K_p=25$, $K_i=0.1$, and $K_d=2.5$ was used as the baseline. All tracking metrics were computed over the common 10--40~s evaluation window after controller initialization.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.96\textwidth]{pictures/figure3_control_tracking.pdf}
...
Table~\ref{tab:control_metrics} summarizes the quantitative tracking results. FS-EDMD-LQR reduces the RMSE$^{\ref{fn:track_metrics}}$/MAE by 67.5\%/66.2\% for sinusoidal tracking and by 57.7\%/56.0\% for irregular tracking, confirming its advantage over PID.
...
\midrule
\multirow{2}{*}{Soft: spiral}
    & FS-EDMD-LQR & \textbf{0.2405} & \textbf{0.1983} \\
    & PID & 1.1407 & 0.9668 \\
\midrule
\multirow{2}{*}{Soft: five-point star}
    & FS-EDMD-LQR & \textbf{0.1321} & \textbf{0.1278} \\
    & PID & 0.6313 & 0.6202 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{pictures/figure4_soft_control_tracking.pdf}
\caption{Soft-platform trajectory tracking and instantaneous tracking error on the spiral and five-point-star references. The top row compares Reference, FS-EDMD-LQR, and PID trajectories; the bottom row shows the pointwise Euclidean tracking error after the common initial approach interval.}
...
FS-EDMD-LQR follows both the spiral and five-point-star references more
closely than PID. As reported in Table~\ref{tab:control_metrics},
FS-EDMD-LQR reduces the RMSE$^{\ref{fn:track_metrics}}$/MAE by
78.9\%/79.5\% on the spiral trajectory and by 79.1\%/79.4\% on the

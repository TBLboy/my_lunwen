...
Extended Dynamic Mode Decomposition with Dictionary Learning (EDMDDL) learns a
...
selection matrix parameterizes this recombination, and the resulting
feature-selection-based EDMD (FS-EDMD) model is trained directly for lifted
...
\begin{figure}[pos=t]
\centering
\includegraphics[width=0.75\columnwidth]{pictures/Figure3.pdf}
...
Fig.~\ref{fig:robotic_modeling} shows the prediction results on the robotic arm. FS-EDMD maintains the closest agreement with the measured joint trajectory over the 3.2~s prediction horizon, whereas EDMDDL gradually drifts and standard EDMD accumulates larger errors.

\begin{figurehere}
\centering
\includegraphics[width=\columnwidth]{pictures/Figure4.pdf}
\caption{Modeling accuracy on the robotic arm. The panels show Joint 1--3 predictions over the 800-step rollout.}
\label{fig:robotic_modeling}
\end{figurehere}

For the soft platform, Fig.~\ref{fig:soft_modeling} shows the $x$ and $y$ predictions on test trajectory 66. The FS-EDMD predictions follow the measured light-spot trajectory more closely than the two baselines over the displayed test trajectory.

\begin{figurehere}
\centering
\includegraphics[width=\columnwidth]{pictures/Figure5.pdf}
\caption{Modeling accuracy on the cable-driven soft platform for test trajectory 66, showing the $x$ and $y$ predictions.}
\label{fig:soft_modeling}
\end{figurehere}

As shown in Table~\ref{tab:modeling_accuracy}, FS-EDMD achieves the lowest full-state prediction errors on the robotic arm, with MSE, RMSE, and MAE of 0.3520, 0.5933, and 0.3202, respectively.\footnotemark These correspond to reductions of 40.7\%, 23.0\%, and 29.6\% over EDMDDL, and 51.5\%, 30.3\%, and 36.2\% over standard EDMD. On the soft platform, FS-EDMD also achieves the lowest errors on test trajectory 66, with an MSE/RMSE/MAE of 0.6224/0.7889/0.5767. These correspond to reductions of 22.6\%/12.0\%/19.3\% over standard EDMD and 62.8\%/39.0\%/38.4\% over EDMDDL.
...
Robustness was also evaluated within the same arm-tracking setup using the sinusoidal reference and the same controller parameters. During the 30~s experiment, an operator manually disturbed the robotic arm by briefly pushing and pulling the links associated with the three joints, thereby introducing unknown bounded joint torques. In the disturbance-recovery row of Fig.~\ref{fig:control_tracking}, the joint trajectories exhibit temporary deviations from the references during the disturbances but recover smoothly after the disturbances are removed and remain bounded throughout the experiment.

Table~\ref{tab:control_metrics} summarizes the tracking results.\footnotemark FS-EDMD-LQR reduces the RMSE$^{\ref{fn:track_metrics}}$/MAE by 67.5\%/66.2\% for sinusoidal tracking and by 57.7\%/56.0\% for irregular tracking, confirming its advantage over PID.
...
\caption{Soft-platform trajectory tracking on the triangle and Lissajous references. The panels compare the complete Reference, FS-EDMD-LQR, and PID trajectories.}
\label{fig:soft_control_tracking}
\end{figurehere}

For the soft platform, the PID baseline used \(K_p=20.00\), \(K_i=1.00\),
and \(K_d=10.00\), and the FS-EDMD-LQR controller used
\(\mathbf{Q}_x=\diag(250,250)\) and \(\mathbf{R}=\mathbf{I}_2\). Both
controllers used the same reference trajectory, initial condition, sampling
period, and endpoint condition.
...
FS-EDMD-LQR follows both the triangle and Lissajous references more
closely than PID. As reported in Table~\ref{tab:control_metrics},
FS-EDMD-LQR reduces the RMSE$^{\ref{fn:track_metrics}}$/MAE by
73.7\%/68.9\% on the triangle trajectory and by 92.9\%/93.0\% on the
...
This paper addressed the dictionary-design tradeoff in EDMD through a learnable
feature-selection matrix. Starting from an overcomplete physics-informed
candidate library, the proposed method learns compact feature combinations and
updates the lifted dynamics jointly with the selection matrix. A
feedforward--feedback Koopman-LQR controller was then designed by treating
finite-dimensional approximation errors and external uncertainties as bounded
lumped disturbances. Lyapunov analysis establishes ultimate uniform boundedness
of the closed-loop tracking error. Experiments on a three-degree-of-freedom
rigid robot arm and a cable-driven soft platform show lower multi-step
prediction errors than EDMD and EDMDDL and lower tracking errors than PID over
the tested references. A separate disturbance-recovery test on the robotic arm
further shows bounded deviations and smooth recovery. These results are limited
to the tested operating ranges and bounded-disturbance setting. Future work will
consider online adaptation and model predictive control to account explicitly
